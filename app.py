...
class RegisterCandidate(Resource):
    ...
    def post():
        if request.method=='POST':
            ...
            try:
                details={}                
                BiceId=int(request.form['bice_id'])
                Salutation=str(request.form['salutation'])
                FirstName=str(request.form['first_name'])
                MiddleName=str(request.form['middle_name'])
                LastName=str(request.form['last_name'])
                CountryId=int(request.form['country_id'])
                StateId=int(request.form['state_id'])
                CityID=int(request.form['city_id'])
                DoorNoStreet=str(request.form['door_no_street'])
                Pincode=int(request.form['pincode'])
                DOB=str(request.form['dob'])
                Mobile=int(request.form['mobile_number'])
                MaximumEducation=str(request.form['maximum_education'])
                EduOthers=str(request.form['edu_others'])
                ID_Proof=str(request.form['id_proof'])
                ID_Number=str(request.form['id_number'])
                IdOthers=str(request.form['id_others'])
                BankId=int(request.form['bank_id'])
                BankAccountNumber=str(request.form['bank_account_number'])
                UserId=int(request.form['user_id'])
                CandidateImageName=str(request.form['candidate_image'])
